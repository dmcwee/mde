    ...
    for ring in "${rings[@]}"; do
        for arch in "${archs[@]}"; do
            ...
            if [[ "$proxy"=="" ]]; then
                ...
                echo "Downloading from $url with proxy $proxy and saving to folder $path"
                if ! curl -L -x "$proxy" -o "$tempSavePath" "$url"; then
                    handle_error $? "Failed to download $url" "$tempSavePath" "$downloadedManifestXML"
                fi
                if ! curl -L -x "$proxy" -o "$downloadedManifestXML" "$manifestUrl"; then
                    handle_error $? "Failed to download manifest from $manifestUrl" "$tempSavePath" "$downloadedManifestXML"
                fi
            fi
...
proxy=$(echo "$settings" | jq -r '.proxy')
if [[ $proxy == "null" ]]; then
    proxy=$defaultProxy
fi
...
if [[ "$downloadLinuxUpdates" == true ]]; then
    invoke_download_all_updates "linux" "$downloadPreviewUpdates" "$baseUpdateUrl" "$downloadFolder" "$backupPreviousUpdates" "$proxy"
fi

if [[ "$downloadMacUpdates" == true ]]; then
    invoke_download_all_updates "mac" "$downloadPreviewUpdates" "$baseUpdateUrl" "$downloadFolder" "$backupPreviousUpdates" "$proxy"
fi
